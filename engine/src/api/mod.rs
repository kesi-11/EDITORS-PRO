...
pub struct EditorsProEngine {
    ...
    /// Phase F.4: per-track mixer state (pan, solo) keyed by track ID.
    /// Volume + mute are already on TrackInfo; pan + solo live here because
    /// they're newer and adding them to TrackInfo would break the bridge DTO.
    /// video: pan + solo on a separate map, fold into TrackInfo when the
    /// bridge DTO is next regenerated by flutter_rust_bridge_codegen.
    track_mixer_state: std::collections::HashMap<String, TrackMixerState>,
    /// Phase F.4: per-track EQ settings keyed by track ID.
    track_eq_settings: std::collections::HashMap<String, crate::audio::effects::EqSettings>,
    /// Phase F.4: last computed loudness reading. Updated whenever the
    /// engine mixes audio (preview playback or export). Polled by the
    /// Flutter Audio Meter Bridge via `get_current_loudness` FFI.
    /// video: single last-reading field, upgrade to sliding-window
...
pub struct TrackMixerState {
    ...
    pub pan: f32,
    /// When true, all non-soloed tracks are muted during mixing.
    pub solo: bool,
}
...
impl EditorsProEngine {
    ...
    pub fn mix_audio_at_time(
        ...
        for track in audio_tracks {
            for clip in &track.clips {
                ...
                // Phase F.4: pull per-track mixer state (pan, solo, EQ)
                let track_state = self.track_mixer_state.get(&track.id);
                let pan = track_state.map(|s| s.pan).unwrap_or(0.0);
                let solo = track_state.map(|s| s.solo).unwrap_or(false);
                let eq_settings = self.track_eq_settings.get(&track.id).cloned();
        ...
        for track in video_tracks {
            for clip in &track.clips {
                ...
                if let Ok(audio) = self.get_audio_samples(&clip.asset_id) {
                    ...
                    // Phase F.4: pull per-track mixer state for video tracks too
                    let track_state = self.track_mixer_state.get(&track.id);
                    let pan = track_state.map(|s| s.pan).unwrap_or(0.0);
                    let solo = track_state.map(|s| s.solo).unwrap_or(false);
                    let eq_settings = self.track_eq_settings.get(&track.id).cloned();
    ...
    pub fn analyze_samples_loudness(&self, samples: &[f32], sample_rate: u32, channels: u32) -> LoudnessResult {
        ...
        // Cache as the "current" reading for the audio meter bridge.
        if let Ok(mut guard) = self.last_loudness.lock() {
    ...
    pub fn set_track_pan(&mut self, track_id: &str, pan: f32) {
        let state = self.track_mixer_state.entry(track_id.to_string()).or_default();
        state.pan = pan.clamp(-1.0, 1.0);
    }

    /// Get the pan for a track. Returns 0.0 (center) if not set.
    pub fn get_track_pan(&self, track_id: &str) -> f32 {
        self.track_mixer_state.get(track_id).map(|s| s.pan).unwrap_or(0.0)
    }

    /// Set the solo flag for a track. When any track is soloed, all
    /// non-soloed tracks are muted during mixing.
    pub fn set_track_solo(&mut self, track_id: &str, solo: bool) {
        let state = self.track_mixer_state.entry(track_id.to_string()).or_default();
        state.solo = solo;
    }

    /// Get the solo flag for a track. Returns false if not set.
    pub fn get_track_solo(&self, track_id: &str) -> bool {
        self.track_mixer_state.get(track_id).map(|s| s.solo).unwrap_or(false)
    }

    /// Returns true if any track is soloed. Used by the mixer to decide
    /// whether to mute non-soloed tracks.
    pub fn any_track_soloed(&self) -> bool {
        self.track_mixer_state.values().any(|s| s.solo)
    }

    // ─── Phase F.4: Per-track EQ settings ──────────────────────────────

    /// Set the EQ settings for a track. Stored per-track; applied during
    /// mixing when the track is rendered.
    pub fn set_track_eq_settings(&mut self, track_id: &str, settings: crate::audio::effects::EqSettings) {
        self.track_eq_settings.insert(track_id.to_string(), settings);
    }

    /// Get the EQ settings for a track. Returns None if no EQ is configured.
    pub fn get_track_eq_settings(&self, track_id: &str) -> Option<&crate::audio::effects::EqSettings> {
        self.track_eq_settings.get(track_id)
    }

    // ─── Phase F.4: Audio cache write-back + current loudness ──────────
    ...
    pub fn set_audio_samples(&mut self, asset_id: &str, samples: Vec<f32>, sample_rate: u32, channels: u32) {
        if let Some(buffer) = self.audio_cache.get_mut(asset_id) {
            buffer.samples = samples;
            buffer.sample_rate = sample_rate;
            buffer.channels = channels;
        } else {
            // Insert a new buffer if the asset isn't cached yet.
            let duration_ms = (samples.len() as f64 / channels as f64 / sample_rate as f64 * 1000.0) as u64;
            let mut buffer = crate::audio::mixer::AudioBuffer::new(sample_rate, channels, duration_ms);
            buffer.samples = samples;
            self.audio_cache.insert(asset_id.to_string(), buffer);
        }
    }

    /// Get the last computed loudness reading. Returns None if no audio
